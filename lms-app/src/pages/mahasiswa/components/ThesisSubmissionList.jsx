...
import { fetchThesisProposals, checkActiveSubmission, submitThesisTitle, uploadThesis, fetchMahasiswaByNim, checkSubmittedThesis, fetchLecturers, fetchTopicsByLecturer, submitThesisProposal } from '../service/bankService';
import {
  Box,
  Card,
  CardContent,
...
const ThesisSubmissionList = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log('Fetched mahasiswa:', mahasiswaResponse); // Debug log
        ...
        console.log('Approved proposal:', approved); // Debug log
  ...
  const handleOpenTitleModal = (proposal) => {
  ...
  const handleOpenUploadModal = (proposal) => {
    console.log('Opening upload modal for proposal:', proposal); // Debug log
    setSelectedProposal(proposal);
  ...
  const handleCloseProposalModal = () => {
    ...
    setProposalData({ lecturerId: '', topicId: '' });
    setTopics([]);
    setSubmissionError(null);
  };
  ...
  const handleSubmitProposal = async () => {
    ...
    try {
      ...
      console.log('Approved proposal after submit:', approved); // Debug log
      setApprovedProposal(approved || null);
  ...
  const handleSubmitTitle = async () => {
    if (!mahasiswa || !mahasiswa.id) {
      setSubmissionError('Data mahasiswa tidak ditemukan atau ID tidak valid. Silakan login ulang.');
      return;
    }
    try {
      setSubmitting(true);
      setSubmissionError(null);
      await submitThesisTitle({
      ...
      setHasSubmittedThesis(submittedThesis);
      const approved = response.data.find(proposal => proposal.status === 'approved');
      console.log('Approved proposal after title submit:', approved); // Debug log
      setApprovedProposal(approved || null);
      console.log('After submit title:', {
        activeSubmission: activeSubmissionResponse.data,
        hasSubmittedThesis: submittedThesis,
        approvedProposal: approved,
      });
    } catch (err) {
      setSubmissionError('Gagal mengajukan judul skripsi: ' + err.message);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUploadThesis = async () => {
    ...
    if (!mahasiswa.nim || typeof mahasiswa.nim !== 'string' || mahasiswa.nim.trim() === '') {
      console.error('Invalid mahasiswa.nim:', mahasiswa.nim); // Debug log
      setSubmissionError('NIM mahasiswa tidak valid. Silakan login ulang.');
      return;
    }
    if (!approvedProposal) {
      setSubmissionError('Pengajuan skripsi yang disetujui tidak ditemukan.');
      return;
    }
    if (!selectedProposal || !selectedProposal.id) {
      console.error('Invalid selectedProposal:', selectedProposal); // Debug log
      setSubmissionError('Proposal tidak valid. Silakan pilih ulang.');
      return;
    }
    if (selectedProposal.id !== approvedProposal.id) {
      console.error('Selected proposal does not match approved proposal:', { selectedProposal, approvedProposal }); // Debug log
    ...
    try {
      ...
      setHasSubmittedThesis(submittedThesis);
      const approved = response.data.find(proposal => proposal.status === 'approved');
      console.log('Approved proposal after upload:', approved); // Debug log
      setApprovedProposal(approved || null);
  ...
  const isButtonDisabled = (proposal) => {
    ...
    if (hasSubmittedThesis && approvedProposal.dosenId !== proposal.dosenId) return true;
    return false;
  };

  const isUploadButtonDisabled = (proposal) => {
    if (!mahasiswa || !approvedProposal || !mahasiswa.nim) return true;
    if (approvedProposal.dosenId !== proposal.dosenId) return true;
    return !hasSubmittedThesis;
  };

  const isModalUploadButtonDisabled = () => {
    const isFormFilled = uploadData.title && uploadData.author && uploadData.year && uploadData.file && uploadData.abstract;
  ...
  return (
    ...
    >
                    ...
                    <TableCell>
                      {proposal.status === 'approved' && (
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <Button
                            variant="contained"
                            onClick={() => handleOpenTitleModal(proposal)}
      ...
      <Modal open={titleModalOpen} onClose={handleCloseTitleModal}>
        <Box sx={modalStyle}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 3 }}>
            <Button
              ...
              disabled={submitting || !title || !description || !mahasiswa || !mahasiswa.id}
              sx={{ flex: 1 }}
            >
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 3 }}>
              <Button
                ...
                disabled={submitting || !proposalData.lecturerId || !proposalData.topicId}
                sx={{ flex: 1 }}
              >
                {submitting ? <CircularProgress size={24} /> : 'Kirim'}
              </Button>
