 ...
 * @param {Object} proposalData - Proposal data containing lecturerId and topicId
 ...
 */
export const fetchThesisProposals = async () => {
  try {
    const url = `${API_BASE_URL}/proposals?populate=*`;
    console.log('Fetching proposals from:', url); // Debug log
...
export const fetchProposalById = async (proposalId) => {
  try {
    ...
    console.log('Proposal fetch response:', { status: response.status, body: responseText }); // Debug log
    ...
    console.log('Proposal data fetched:', data.data); // Debug log
    return data.data;
  } catch (error) {
